...
function expStart(subject, session) {
    ...
    const usedSequence = sequences[(subjectID-1)%24]
    ...
    jsPsych.init({
        ...
        on_close: function () {
            jsPsych.data.get().localSave("csv", `ASRT_Subject_${subjectID}_Session_${sessionNumber}_quitted_output.csv`); //saves experiment output to .csv file if quitted earlier
        },
        on_finish: function () {
            jsPsych.data.get().localSave("csv", `ASRT_Subject_${subjectID}_Session_${sessionNumber}_output.csv`); //saves experiment output to .csv file
        }
    })
}